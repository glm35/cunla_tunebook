...
def dump_default_bar (outf):
	...
	"""
	# patch glm: I don't like this (no consistency check (?), bars missing
	# before alternatives
...
# (glm) problem: [1 out of a bar is seen as the beginning of a chord...
...
bar_dict = {
 ...
 '|1' : '} \\alternative {{',
 '[1' : '} \\alternative {{',
 ':|2' : '} {', # (glm) '|2' alone seems illegal to me
 '[2' : '{',
 '|' :  '|' # (glm) bar check
  }

alternative_opener = ['|1', '[1', ':|2', '[2']
repeat_ender = ['::', ':|', ':|2', ':|[2']
repeat_opener = ['::', '|:']
in_repeat = [''] * 8
doing_alternative = [''] * 8
using_old = '' # (glm) never set, but used in a few places => should be removed

def try_parse_bar (str,state):
	global in_repeat, doing_alternative
	bs = None
	if current_voice_idx < 0:
		select_voice ('default', '')
	...
	for trylen in [3,2,1]:
		if str[:trylen] and bar_dict.has_key (str[:trylen]):
			...
			if s in repeat_opener:
				# if an alternative is in progress, close it
				if doing_alternative[current_voice_idx] == '2':
					print "Closing the 2nd alternative"
					bs = "} }" + bs
					doing_alternative[current_voice_idx] = ''
				print "Repeat opener found : %s" % s
				in_repeat[current_voice_idx] = 't'

			if s in alternative_opener:
				print "Alternative opener found : %s" % s
				doing_alternative[current_voice_idx] = s[-1]
			else:
				# close the alternative one measure after the 2nd alternative opener
				if doing_alternative[current_voice_idx] == '2':
					print "Closing the 2nd alternative"
					bs = bs + "} }"
					doing_alternative[current_voice_idx] = ''
...
def try_parse_chord_delims (str, state):
	# (glm) check that it is not a first or second repeat
	if len(str) >= 2:
		if str[:2] == '[1' or str[:2] == '[2':
			return str
